...
def run_kilosort(settings, probe=None, probe_name=None, filename=None,
    ...
    print(f"[CALVIN DEBUG] Starting Kilosort. All of my debug messages will be prefixed with [CALVIN DEBUG].")
    ...
    try:
        ...
        clu, Wall = cluster_spikes(results_dir, st, tF, ops, device, bfile, logger, tic0=tic0,
                                progress_bar=progress_bar)
...
def detect_spikes(ops, results_dir, device, bfile, tic0=np.nan, progress_bar=None):
    ...
    clu, Wall = clustering_qr.run(ops, st0, tF, mode='spikes', device=device,
                                  progress_bar=progress_bar)
    Wall3 = template_matching.postprocess_templates(Wall, ops, results_dir, clu, st0, device=device)
    ...
    logger.info('-'*40)
    print(f"[CALVIN DEBUG] Entering extract(). Wall3 (postprocessed templates) = {Wall3} with shape {Wall3.shape}")
    np.save(results_dir / "Wall3.npy", np.array(Wall3.cpu()))
    np.save(results_dir / "wPCA.npy", np.array(ops['wPCA'].cpu()))
    st, tF, ops = template_matching.extract(ops, results_dir, bfile, Wall3, device=device,
                                                 progress_bar=progress_bar)
...
def cluster_spikes(results_dir, st, tF, ops, device, bfile, logger, tic0=np.nan, progress_bar=None):
    tic = time.time()
    logger.info(' ')
    ...
    logger.info('-'*40)
    Wall, clu, is_ref = template_matching.merging_function(ops, results_dir, Wall, clu, st[:,0],
                                                           device=device)
