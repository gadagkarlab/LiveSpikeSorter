...
def run(ops, st, tF,  mode = 'template', device=torch.device('cuda'), progress_bar=None):
        ...
        for jj in np.arange(len(xcent)):
            ...
            if Xd is None:
                ...
                if torch.isnan(Xd).any():
                    raise ValueError("Xd contains NaN values.")
                if torch.isinf(Xd).any():
                    raise ValueError("Xd contains infinite values.")
                if mode == 'template':
                ...
                print(f"[CALVIN DEBUG] type(iclust_init) = {type(iclust_init)}")
                print(f"[CALVIN DEBUG] iclust_init.shape = {iclust_init.shape}")
                xtree, tstat, my_clus = hierarchical.maketree(M, iclust, iclust0)
    ...
    print(f"[CALVIN DEBUG] xcent.shape = {xcent.shape}")
    print(f"[CALVIN DEBUG] ycent.shape = {ycent.shape}")
    return clu, Wall
