...
from pathlib import Path # CALVIN DEBUG
...
def extract(ops, results_dir, bfile, U, device=torch.device('cuda'), progress_bar=None):
    ...
    np.save(results_dir / "ctc.npy", np.array(ctc.cpu()))
...
def postprocess_templates(Wall, ops, results_dir, clu, st, device=torch.device('cuda')):
    Wall2, _ = align_U(Wall, ops, device=device)
    #Wall3, _= remove_duplicates(ops, Wall2)
    Wall3, _, _ = merging_function(ops, results_dir, Wall2.transpose(1,2), clu, st[:,0],
...
def calvin_debug_print_ops(ops):
    for key in ops:
        if isinstance(ops[key], (bool, str, int, float, type(None))):
            print(f"[CALVIN DEBUG] ops[{key}] = {ops[key]}")

def run_matching(ops, X, U, ctc, device=torch.device('cuda')):
    global wrote_tensors # CALVIN DEBUG
    Th = ops['Th_learned']
    nt = ops['nt']
    W = ops['wPCA'].contiguous()

    calvin_debug_print_ops(ops)
    print(f"[CALVIN DEBUG] batch shape = {X.shape}")
    print(f"[CALVIN DEBUG] ctc.max() = {ctc.max()}")
    print(f"[CALVIN DEBUG] Wall3.max() = {U.max()}")
    print(f"[CALVIN DEBUG] Th_learned = {Th}")
    print(f"[CALVIN DEBUG] wPCA.max() = {W.max()}")
...
def merging_function(ops, results_dir, Wall, clu, st, r_thresh=0.5, mode='ccg', device=torch.device('cuda')):
    clu2 = clu.copy()
    ...
    np.save(results_dir / "imap.npy", imap)
